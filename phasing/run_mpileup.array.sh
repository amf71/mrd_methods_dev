...
#bcftools view -v snps --min-af 0.25 --max-af 0.75 ${SNP_VCF_N} > ${SCRATCH_DIR}/${N_SAMPLE}_filt_hetsnps_mpileup.vcf
#
...
# include indels for the phasing above but only actually use SNPs in the mpileup
bcftools view -v snps ${SCRATCH_DIR}/${N_SAMPLE}_filt_hetsnps_mpileup.vcf > ${SCRATCH_DIR}/${N_SAMPLE}_filt_hetsnps_noindel_mpileup.vcf
grep -v '#' ${SCRATCH_DIR}/${N_SAMPLE}_filt_hetsnps_noindel_mpileup | awk -v OFS="\t" ' {print $1, $2}' > ${SCRATCH_DIR}/${N_SAMPLE}.hetsnps.bed
